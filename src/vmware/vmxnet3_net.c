...
vmxnet3_check_version(vmxnet3_pci dev)
{
    ...
    if ((version & 0x01) == 0) {
        rprintf("unsupported hardware version %x\n", version);
        assert(false);
    }
    ...
    if ((version & VMXNET3_UPT_VERSION) == 0) {
        rprintf("unsupported UPT version %x\n", version);
        assert(false);
    }
...
vmxnet3_net_attach(heap general, heap page_allocator, pci_dev d)
{
    ...
    pci_setup_msix(dev->dev, 0, vn->tx_intr_handler, "vmxnet3 tx");
...
//            stats.rx_drops++;
       ...
       if (rxcd->sop) {
        ...
        if (rxcd->eop) {
            if (vdev->n->input(dev->currpkt_head, vdev->n) != ERR_OK) {
                receive_buffer_release(dev->currpkt_head);
            }
            dev->currpkt_head = dev->currpkt_tail = NULL;
        }
